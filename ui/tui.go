...
// Added a photo in base64 format to variable photo
...
func Greet() {
	...
	flex.SetDirection(tview.FlexRow). // Adjusting jokelist & jokeText
...
func addJokeList() { // storing the data given through the forms
	jokesList.Clear()
...
// this form is for the option `generate a random joke`
...
// this form is for the option `get n no of jokes`
...
func setConcatText(contact *Datas) { // Adding text to jokeTest to view
	jokeText.Clear()
	text := "Hey " + contact.firstName + " ready to laugh?\n"
	if contact.number != "" { // for choose option
		...
		text += cmd.GetRandomJoke() // for random joke option
	}
